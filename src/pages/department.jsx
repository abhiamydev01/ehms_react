...
const fetchTransactions = async () => {
  ...
  try {
    ...
    if (Array.isArray(departments)) {
      ...
      setTotalRows(departments.length);
    } else {
      setData([]);
      setFilteredData([]);
      setTotalRows(0);
  ...
  setLoading(false);
    ...
    const columns = [
        { name: "Code", selector: (row) => row.department_code, sortable: true },
        { name: "Name", selector: (row) => row.department_name, sortable: true },
        { name: "Date", selector: (row) => row.createdAt },
        {name:"Head", selector: (row) => row.head_of_department?.name},
    ...
    return (
        <div className="bg-white rounded-sm border border-gray-200 shadow-xs p-4 overflow-x-auto">
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <NavLink
                    to={`/${encryptRoute("master")}/${encryptRoute("create-department")}`}
                >
