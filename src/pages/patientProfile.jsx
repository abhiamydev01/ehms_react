...
export default function AppointmentView() {
    ...
    console.log("Card data from state", card);
    ...
    const fetchMedicalHistory = async () => {
        try {
            setLoading(true);
            const payload = { patientId: card?._id };
    ...
    useEffect(() => {
        if (card?._id) fetchMedicalHistory();
    }, [card?._id]);
    ...
    const getLastVitalsDetails = async () => {
        try {
            setLoading(true);
            const payload = { patient_id: card?._id };
        ...
        try {
            const payload = {
                ...
                vitals: vitalsList.map(v => ({
                    parameter_id: v.unit?.value || "",
                    patient_range: v.range || ""
                }))
            };
            ...
            <div className="bg-white border border-gray-200 rounded-sm shadow-xs flex flex-col md:flex-row justify-between items-center overflow-hidden mb-3">
                ...
                <div className="flex items-center px-4 py-4 space-x-6">
                    ...
                    <div className="text-left">
                        <div className="text-sm font-medium text-indigo-500"> {card?.code || "Unknown"}</div>
                        <h2 className="text-md font-medium">{card?.name || "Unknown"}</h2>
                        ...
                        <div className="text-sm text-gray-600 mt-3 flex items-center space-x-3">
                            <span className="flex items-center space-x-1">
                                ...
                                <span className="font-medium">{card?.phone || "Unknown"}</span>
                            </span>
                            <span className="flex items-center space-x-1">
                                <FaCalendarAlt className="text-gray-400" />
                                <span>
                                    Last Visited : <span className="font-medium"> {previousAppointments?.[0]?.appointmentDate ?? ""}
                                    </span>
                                </span>
                            </span>
                        </div>
                ...
                <div className="flex flex-col items-center space-y-3 p-4">
                    ...
                    <div className="inline-block text-left">
                        <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm text-gray-700 mt-3">
                            <p><span className="font-medium">Age:</span> {card?.age || "N/A"}</p>
                            <p><span className="font-medium">DOB:</span> {card?.dob || "N/A"}</p>
                            <p><span className="font-medium">Blood Group:</span> {card?.bloodGroup || "N/A"}</p>
                            <p><span className="font-medium">Gender:</span> {card?.gender || "N/A"}</p>
                            <p><span className="font-medium">Relation:</span> {card?.relation || "N/A"}</p>
                        </div>
                        {/* {(role === "nurse")&&(
            <button
              ref={buttonRef}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs font-semibold text-gray-400 text-left">
                                            {previousAppointments?.[0]?.appointment_type ?? ""}
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs font-semibold text-gray-400 text-left">
                                            {previousAppointments?.[0]?.doctor_id?.name ?? ""}
                        ...
                        <div className="flex justify-end mb-3">
                            ...
                            >
                                <form className="space-y-4" onSubmit={handleVitalsSubmit}>
                                        ...
                                        <div key={index} className="flex items-center space-x-2 mb-2">
                                            ...
                                            <div className="flex-1">
                                                <Select
                                                    ...
                                                    value={vital.unit}
                                                    onChange={(selected) => {
                                                        const newList = [...vitalsList];
                                                        newList[index].unit = selected;
                                                ...
                                                {vital.unit && (
                                                    <div className="mt-1 text-xs text-blue-500">
                                                        Normal: {vital.unit.normal_range} {vital.unit.unit} | Global: {vital.unit.global_range} {vital.unit.unit}
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    const newList = [...vitalsList];
                                                    newList[index].range = e.target.value;
                                                    setVitalsList(newList);
                                                }}
                                                placeholder="Enter Patient Range"
                                            ...
                                            {vitalsList.length > 1 && (
                                                <button
                                                    type="button"
                                                    onClick={() => setVitalsList(vitalsList.filter((_, i) => i !== index))}
                                                    className="text-red-500 text-sm px-2 py-1 border rounded"
                                                >
                                                    Remove
                                                </button>
                                            )}
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setVitalsList([...vitalsList, { unit: null, range: "" }])}
                                    ...
                                    <div className="flex justify-end mt-4">
                                        <button
                                            type="submit"
                                            disabled={loading}
                                            className={`bg-blue-500 border text-white px-2 py-1 rounded-sm flex items-center justify-center gap-1
          ${loading ? "opacity-50 cursor-not-allowed" : "hover:text-blue-500 hover:bg-transparent cursor-pointer"}`}
                                        >
                                            <LuCircleCheckBig />
                                            <span className="text-sm font-medium">
