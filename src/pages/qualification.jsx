...
const fetchTransactions = async () => {
  setLoading(true);
  try {
    const res = await ApiService.getAllQualification();

    const qualification = res.data?.data || [];
    console.log("Qualification API Response:", qualification);

    if (Array.isArray(qualification)) {
      setData(qualification);
      setFilteredData(qualification);
      setTotalRows(qualification.length);
    } else {
      setData([]);
      setFilteredData([]);
      setTotalRows(0);
  ...
  setLoading(false);
    ...
    const columns = [
        ...
        { name: "Abbrevetion", selector: (row) => row.abbreviation, sortable: true },
    ...
    return (
        <div className="bg-white rounded-sm border border-gray-200 shadow-xs p-4 overflow-x-auto">
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <NavLink
                    to={`/${encryptRoute("master")}/${encryptRoute("create-qualification")}`}
                >
